...
export default function CalificarAlumnos() {
    ...
    useEffect(() => {
        if (idCurso) {
            ...
            getMaterialesPendientes(idCurso).then((data) => setMateriales(data || []));
    ...
    const handleOpenStudentList = async (material) => {
        ...
        try {
            ...
            if (!material.formulario_id) {
                alert("Error: Material sin formulario ID (backend logic info missing)");
                return;
            }
    ...
    const handleSubmitExam = async () => {
        ...
        try {
            ...
            if (result && result.nota !== undefined) {
                alert(`Calificación guardada para ${selectedStudent.nombre}. Nota: ${result.nota}`);

                // Remove student from list immediately
                setAlumnos(prev => prev.filter(a => a.id !== selectedStudent.id));

                // Check if list is now empty? If so, remove material from list?
                if (alumnos.length <= 1) { // <= 1 because state update allows it to be 0 next render
                    // Re-fetch materials to see if this material should disappear
                    getMaterialesPendientes(idCurso).then(data => setMateriales(data || []));
                    setActiveModal(null); // Close modal if no students left
                } else {
                    setActiveModal('students');
                }
    ...
    return (
        <div className="calificar-alumnos-page" style={{ padding: "20px" }}>
            ...
            <div style={{ marginTop: "20px" }}>
                ...
                {materiales.length > 0 ? (
                    materiales.map((m, i) => (
                        <div key={i} style={{ padding: "10px", borderBottom: "1px solid red", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                            <span>{m.titulo}</span>
                            <button
                    ...
                    <p>Todos los materiales están calificados o no hay materiales.</p>
                )}
            </div>

            {/* Modal: List of Students */}
            {activeModal === 'students' && (
                <Modal isOpen={true} onClose={() => setActiveModal(null)} title={`Alumnos Pendientes - ${selectedMaterial?.titulo}`}>
                    {alumnos.length > 0 ? (
                        <table style={{ width: "100%", borderCollapse: "collapse" }}>
                            ...
                            <tbody>
                                {alumnos.map((alum) => (
                                    <tr key={alum.id} style={{ borderBottom: "1px solid #eee" }}>
                                        <td style={{ padding: "10px" }}>{alum.nombre} {alum.apellido}</td>
            ...
            {activeModal === 'exam' && formulario && selectedStudent && (
                <Modal isOpen={true} onClose={() => setActiveModal('students')} title={`Evaluando a: ${selectedStudent.nombre} ${selectedStudent.apellido}`}>
                    <div className="exam-container" style={{ maxHeight: '70vh', overflowY: 'auto' }}>
